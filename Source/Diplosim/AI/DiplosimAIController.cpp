...
{
	if (!IsValid(Actor) || MoveRequest.GetGoalActor() != Actor || (!Cast<AAI>(GetOwner())->MovementComponent->Points.IsEmpty() && FVector::Dist(Actor->GetActorLocation(), Cast<AAI>(GetOwner())->MovementComponent->Points.Last()) < 20.0f))
		return;
